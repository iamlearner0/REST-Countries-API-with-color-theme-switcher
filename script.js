...
getCountries().then((countries) => {
  countries.forEach((thecountry) => {
    ...
    countryRegion.innerHTML = `Region: <span>${thecountry.region}</span>`;

    let countryCapital = document.createElement("p");
    countryCapital.innerHTML = `Capital: <span>${thecountry.capital ? thecountry.capital[0] : 'N/A'}</span>`;
